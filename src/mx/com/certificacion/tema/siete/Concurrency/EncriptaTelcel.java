...
public class EncriptaTelcel {
    ...
    public String encripta(String s, String s1) {
        ...
        int ai7[] = stringToBin(s);
        int ai1[] = stringToHex(s1);
        int i = 0;
        for (int j = 0; j < 64; j++) {
            if ((j + 1) % 8 != 0) {
                if (ai1[i] == -1) {

                    System.out.println("\nERROR: Key has to be a hex value (0-9, a-f, A-F)");
                    return "";
                }
                ai2[j] = ai1[i++];
            } else {
        ...
        for (int j1 = 0; j1 < 16; j1++) {
            ai6[j1 + 1] = xor32(func(ai6[j1], ai3[j1 + 1]), ai5[j1]);
            ai5[j1 + 1] = ai6[j1];
        }
        ...
        int ai[] = inverseInitialPermutation(ai7);
